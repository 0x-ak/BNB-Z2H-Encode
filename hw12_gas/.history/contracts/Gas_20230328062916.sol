...
pragma solidity 0.8.0;
    ...
    uint32 constant tradeFlag = 1;
    uint32 constant basicFlag = 0;
    uint32 constant dividendFlag = 1;
...
contract GasContract  {
    ...
    modifier checkIfWhiteListed(address sender) {
        ...
        require(
            senderOfTx == sender,
            "Gas Contract CheckIfWhiteListed - origin not sender"
        );
        uint256 usersTier = whitelist[senderOfTx];
        require(
            usersTier > 0,
            "Gas Contract CheckIfWhiteListed user is not whitelisted"
        );
        require(
            usersTier < 4,
            "Gas Contract CheckIfWhiteListed modifier : user's tier is incorrect"
        );
        _;
    }
